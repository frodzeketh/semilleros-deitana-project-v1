...
PARA SABER EL RANGO DE TARIFA SEGUN CANTIDAD DE PLANTAS QUE UN CLIENTE QUIERA POR NOMBRE DE FAMILIA: 
SELECT 
    f.id AS codigo_familia,
    f.FM_DENO AS familia,
    fr.id2 AS numero_rango,
    fr.C0 AS cantidad_minima,
    fr.C1 AS cantidad_maxima,
    fr.C2 AS tarifa,
    CASE 
        WHEN fr.C1 = 0 THEN CONCAT(fr.C0, ' o más plantas')
        ELSE CONCAT(FORMAT(fr.C0, 0), ' - ', FORMAT(fr.C1, 0), ' plantas')
    END AS rango_descripcion
FROM familias f
INNER JOIN familias_fm_rngt fr ON f.id = fr.id
WHERE f.FM_DENO LIKE '%PEPINO%'
ORDER BY fr.id2;
